...
public class LightEmitter
	...
	public LightEmitter(Vector2f position, float size, float rotation, boolean movable, boolean rotatable, Color color,
			Vector2f rPosition, Float rDistance, Float rAngle, Float rSpectrum, Image image) {
		super();
	...
	public void setPosition(Vector2f v) {
		position = v;
		lightRay.positions.get(0).set(v);
	}
	...
	public IControllable clone() {
		return new LightEmitter(Tools.copyIfPresent(position), size, rotation, movable, rotatable, color,
				Tools.copyIfPresent(rPosition), Tools.copyIfPresent(rDistance), Tools.copyIfPresent(rAngle),
				Tools.copyIfPresent(rSpectrum), image);
	}
